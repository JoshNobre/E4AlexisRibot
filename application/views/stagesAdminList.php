...
<html>
	<head>
		...
		<title>Liste des participants aux stages</title>
...
<?php 
    ...
    echo '<div class="col-lg-3">';
        ...
        foreach($this->stagesAdminList->getTournoi() as $row) {
          echo '<option value='.$row->Id_Stage.'>'; echo $row->Nom_Stage; echo '</option>';
        }
        echo '</select>'; 
    ...
    $dateStage = array(
        ...
        echo '<label class="control-label col-lg-5"> Date du stage </label>';
        echo form_input($dateStage);
        echo '<br/>';

        echo '<select name="lieu">';
        foreach($this->stagesAdminList->getLieu() as $row) {
